...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, firstName, lastName, onUpdateProfile, onLogout }) => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 max-w-4xl mx-auto animate-fade-in">
            <h1 className="text-3xl md:text-5xl font-extrabold bg-gradient-to-r from-white via-zinc-200 to-zinc-300 bg-clip-text text-transparent flex items-center gap-3">
                <UserIcon />
                My Profile
            </h1>
            ...
            <div className="glass-card p-4 md:p-6 rounded-xl shadow-sm">
                <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-white border-b border-[rgba(255,255,255,0.1)] pb-3">Personal Information</h2>
            ...
            {/* Security */}
            <div className="glass-card p-4 md:p-6 rounded-xl shadow-sm">
                <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-white border-b border-[rgba(255,255,255,0.1)] pb-3">Security</h2>
            ...
            {/* Logout */}
            <div className="glass-card p-4 md:p-6 rounded-xl shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-lg md:text-xl font-bold text-white">Logout</h2>
                    <p className="text-[#A0A0A0] text-sm">End your current session.</p>
                </div>
                <button
                    onClick={onLogout}
                    className="w-full md:w-auto flex items-center justify-center gap-2 bg-[#DC3545] hover:bg-[#e85d75] text-white font-bold py-3 px-6 rounded-lg 
            ...
            {/* Notification Toast */}
            {notification && (
                 <div className={`fixed bottom-20 md:bottom-5 right-4 md:right-5 left-4 md:left-auto p-4 rounded-xl shadow-lg text-white font-semibold animate-scale-in z-50 ${
